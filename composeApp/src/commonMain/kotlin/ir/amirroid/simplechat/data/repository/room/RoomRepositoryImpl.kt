...
class RoomRepositoryImpl(
    ...
    override suspend fun fetchAndSaveAllRooms(): Response<DefaultRequiredResponse<List<Room>>, NetworkErrors> {
        return RetryCallExecutor.call(times = Int.MAX_VALUE) { getAllRooms() }
            .onSuccess { response ->
                saveRooms(response.data, withClearData = true)
            }
    }

    override suspend fun saveRooms(rooms: List<Room>, withClearData: Boolean) {
        appDatabase.useWriterConnection {
            if (withClearData) roomDao.clearAll()
            val roomEntities = rooms.map { it.toEntity() }

            val memberEntities = rooms.flatMap { room ->
                room.members.map { member -> member.toEntity(room.id) }
            }

            val userEntities = rooms
    ...
    override suspend fun updateLastMessageRoom(roomId: Long, message: String) {
        roomDao.updateLastMessage(roomId, message)
    }
}